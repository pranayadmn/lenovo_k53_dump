...
if [ $SAVEFLAG = TRUE ]; then
	...
	[ -d $DATAAPLOG ] && dumpsys cpuinfo > $DATAAPLOG/cpuinfo.txt
        ...
        [ -d $DATAAPLOG ] && dumpsys batterystats > $DATAAPLOG/batterystats.txt
	[ -d $DATAAPLOG ] && [ -e /d/le_rkm ] && date  > $DATAAPLOG/lk_mesg.txt && cat /d/le_rkm/lk_mesg >> $DATAAPLOG/lk_mesg.txt
	...
	cd $APLOG_DIR && chown -R media_rw:media_rw alarm.txt power.txt cpuinfo.txt dumpmeminfo.txt bluetooth lk_mesg.txt \
